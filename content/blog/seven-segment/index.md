...
I'm using [`volatile_register`](), which provides read-only, write-only and read-write cells with volalile access for modelling CPU registers. Rust really shines here, as I can encode what functions are read-only and what requires a write in the type system, and through the use of immutable vs mutable references. Writes are also effectful, so require `unsafe` blocks, whereas reads do not. Note that the `#[repr(C)]` is needed as Rust makes no guarantees on data layout, so we need to tell the compiler to lay the data out in the way C would (which is the way you would expect) to be able to just cast our address to a `*mut UartRegs` and it work.
...
![IP block in Vivado]( "My very own IP core!")
...
Wanted to impl index, but can't

Example program that does the gif

## Conclusion

This was a really fun little project, getting to play around with loads of different aspects of embedded and digital systems design that I'm interested in, and that I'm going to be doing a lot more of next year. There was a _lot_ of trial and error, poring over Xilinx documentation, and fighting Vivado (when it wasn't crashing), but its those kinds of experiences where you often end up learning the most. It really didn't come together anywhere near as cleanly as I've probably made out :p.
